...
def tsne(X=np.array([]), no_dims=2, initial_dims=50, perplexity=30.0, labels=None, save_gif=False):
    ...
    plt.title("t-SNE Optimization Error Curve")
    plt.grid(True)
    plt.savefig("result/t-SNE_curve.png", dpi=300)
    #plt.show()
    plt.close()

    plot_similarity_distribution(P, Q, method_name="t-SNE")
